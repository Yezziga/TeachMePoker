        ...
        if (currentPlayer == noOfPlayers - 1) {
          if (!gController.getPlayerDecision().equals("fold")
            ...
            if (!(checkLivePlayers() > 1)) {
              ...
              gController.setWinnerLabel(winner, 99, null, null, null);
          ...
          if (!aiPlayers.get(currentPlayer).getDecision().contains("lost")) {
            if (!aiPlayers.get(currentPlayer).getDecision().contains("fold")
              ...
              if (!(checkLivePlayers() > 1)) {
                ...
                gController.setWinnerLabel(winner, 98, null, null, null);
    ...
    if (doAllInCheck) {
      ...
      if (!gController.getPlayerDecision().contains("fold")) {
        ...
        if (gController.getHandStrength() > bestHand) {
          gController.setPlayerPot(currentPotSize);
          winner = gController.getUsername();
          gController.setWinnerLabel(winner, gController.getHandStrength(), mainPlayerCombination.getWinningCards(gController.getRound()), mainPlayerCombination.getAIHand(), mainPlayerCombination.getAllCards());
          ...
          if (gController.getGetHighCard() > bestHandPlayer.getHighCard()) {
            ...
            if (!secWin.isEmpty()) {
              ...
              gController.setWinnerLabel(winner, bestHand,  bestHandPlayer.getWinningCards(), bestHandPlayer.getAIHand(), bestHandPlayer.getAllCards());
            }
          }
    ...
    for (int i = potSplits.length - 1; i >= 0; i--) {
      if (potSplits[i][0] > 0) {
        ...
        if (!gController.getPlayerDecision().contains("fold")
          ...
          if (gController.getHandStrength() > bestHand) { //Player wins
            gController.setPlayerPot(allInPotSize);
            winner = gController.getUsername();     
            
            gController.setWinnerLabel(winner, gController.getHandStrength(), mainPlayerCombination.getWinningCards(gController.getRound()), mainPlayerCombination.getAIHand(), mainPlayerCombination.getAllCards());
            //System.out.println(winner + " " + bestPlayerCardOne + "flowers");
          } else if (gController.getHandStrength() == bestHand) { //Player wins
            if (gController.getGetHighCard() > bestHandPlayer.getHighCard()) {
              gController.setPlayerPot(allInPotSize);
              winner = gController.getUsername();
