...
public class SPController extends Thread {
				...
				if (currentPlayer == noOfPlayers - 1) {
					if (!gController.getPlayerDecision().equals("fold")
						...
						if (!(checkLivePlayers() > 1)) {
							...
							gController.setWinnerLabel(winner, 99, null, null, null, false);
					...
					if (!aiPlayers.get(currentPlayer).getDecision().contains("lost")) {
						if (!aiPlayers.get(currentPlayer).getDecision().contains("fold")
							...
							if (!(checkLivePlayers() > 1)) {
								...
								gController.setWinnerLabel(winner, 98, null, null, null, false);
	...
	private void checkWinner() {
		...
		if (doAllInCheck) {
			...
			AiCalculation mainPlayerCombination = new AiCalculation(gController.getMainPlayerHand(), gController.getTableCards());
			int mainPlayerHandStrength = mainPlayerCombination.getHandStrength(gController.getRound());
			boolean AIWinByHighCard = false;
			/*
			...
			if (!gController.getPlayerDecision().contains("fold")) {
				// Player wins
				if (mainPlayerHandStrength > bestHand) {
					gController.setPlayerPot(currentPotSize);
					winner = gController.getUsername();
					gController.setWinnerLabel(winner, mainPlayerHandStrength, mainPlayerCombination.getWinningCards(gController.getRound()), mainPlayerCombination.getAIHand(), mainPlayerCombination.getAllCards(), false);

					// draw in hand strength
				} else if (mainPlayerHandStrength == bestHand) {
					// Player wins
					if (gController.getGetHighCard() > bestHandPlayer.getHighCard()) {
						...
						if (!secWin.isEmpty()) {
							...
							// Ai wins with high card and there aren't
						} else {
							bestHandPlayer.updateWinner(currentPotSize);
							winner = bestHandPlayer.getName();
							gController.setWinnerLabel(winner, bestHand, bestHandPlayer.getWinningCards(), bestHandPlayer.getAIHand(), bestHandPlayer.getAllCards(), true);         
						}
					}
					...
					if (!secWin.isEmpty()) {
						...
						if(AIWinByHighCard) {
							...
							gController.setWinnerLabel(winner, bestHand, bestHandPlayer.getWinningCards(), bestHandPlayer.getAIHand(), bestHandPlayer.getAllCards(), false);
						}
					}
				}
		...
		for (int i = potSplits.length - 1; i >= 0; i--) {
			if (potSplits[i][0] > 0) {
				...
				if (!gController.getPlayerDecision().contains("fold")
					...
					if (mainPlayerHandStrength > bestHand) { //Player wins
						gController.setPlayerPot(allInPotSize);
						winner = gController.getUsername();     

						gController.setWinnerLabel(winner, mainPlayerHandStrength, mainPlayerCombination.getWinningCards(gController.getRound()), mainPlayerCombination.getAIHand(), mainPlayerCombination.getAllCards(), false);
						//System.out.println(winner + " " + bestPlayerCardOne + "flowers");
					} else if (mainPlayerHandStrength == bestHand) { //Player wins
						if (gController.getGetHighCard() > bestHandPlayer.getHighCard()) {
							gController.setPlayerPot(allInPotSize);
							winner = gController.getUsername();
					...
					} else { //Ai wins with better hand strength than player
						if (!secWin.isEmpty()) {
							int divBy = allInPotSize = secWin.size();
							for (int x : secWin) {
								aiPlayers.get(x).updateWinner(divBy);
